...
import { getJobIndex, IndexedJob } from "./indexer";
 ...
 * ค้นหา keywords ใน title โดย:
 * - ถ้าเจอคำที่ตรงมาก (exact match / ครอบคลุมมาก) = คะแนนสูง
 * - ถ้าเจอคำที่ตรงน้อย (partial match) = คะแนนต่ำ
 */
  ...
  const results = index.map(item => {
    ...
    for (const keyword of userKeywords) {
      ...
      if (jobTitle.includes(keyword)) {
        partialMatches++;
        totalScore += 0.5; // Half score for partial match
        continue;
      }

      // Similar words (fuzzy match - simple)
      const words = jobTitle.split(/\s+/);
      for (const word of words) {
        const similarity = calculateStringSimilarity(keyword, word);
        if (similarity > 0.7) {
          totalScore += similarity * 0.3; // Reduced score for fuzzy match
          break;
        }
      }
    }
  ...
  const results = index.map(item => {
    ...
    for (const userSkill of userSkills) {
      if (primarySkills.includes(userSkill)) {
        matchScore += 1.0; // Full score for benefits match
        matchedCount++;
      } else if (secondarySkills.includes(userSkill)) {
        matchScore += 0.6; // Reduced score for other fields
        matchedCount++;
      }
    }
